...
describe("상품 API", () => {
  ...
  it("부분 수정하면 보내지 않은 필드는 유지된다", async () => {
    const createRes = await request(app)
      .post("/products")
      .set("Authorization", `Bearer ${accessToken}`)
      .send({
        name: "태그상품",
        description: "태그가 있는 상품입니다",
        price: 5000,
        tags: ["중고"],
      });
    expect(createRes.body.tags).toEqual(["중고"]);
    ...
    expect(updateRes.body.name).toBe("이름만수정");
    expect(updateRes.body.tags).toEqual(["중고"]);
  });

  it("수정 body가 유효하지 않으면 400을 반환한다", async () => {
    const res = await request(app)
