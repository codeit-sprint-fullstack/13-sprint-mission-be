...
async function findAll({
  ...
  take,
  orderBy,
}: Pick<Prisma.ProductFindManyArgs, "where" | "skip" | "take" | "orderBy">) {
...
/** 상품 소유자 id 조회 (권한 체크용) */
async function findOwnerId(id: Product["id"]) {
...
/** 상품 수정
 * - 태그 있으면 전체 삭제 후 재생성 */
async function update({ id, data }: { id: Product["id"]; data: ProductInput }) {
...
async function toggleLike({
  ...
  /** Awaited<ReturnType<typeof findLikedProductById>>
   * - 함수로부터 타입을 추론해서 사용
   * - Prisma 스키마가 바뀌어도 수정 불필요함
   * - Awaited: Promise 벗기고 실제 resolve 타입 얻기
   * - ReturnType: 반환 타입만 추출
   * - typeof : 함수타입 가져오기
   */
  hasLikedProduct: Awaited<ReturnType<typeof findLikedProductById>>;
  ownerId: User["id"];
...
async function findLikedProductById({
  ownerId,
  productId,
}: {
  ownerId: User["id"];
...
async function findLikedProductIds({
  ownerId,
  productIds,
}: {
  ownerId: User["id"];
