...
import type { PasswordUpdateRequest, Requester } from '../../types/application.js';

export class UpdateUserPasswordHandler {
    static async handle(requester: Requester, { password, passwordConfirmation, currentPassword }: PasswordUpdateRequest) {
        ...
        // 현재 패스워드가 일치하는지 검증
        if (!UserPasswordBuilder.comparePassword(currentPassword, userEntity.encryptedPassword)) {
