...
import type { SignInRequest } from '../../types/application.js';

export class SignInLocalUserHandler {
    static async handle({ email, password }: SignInRequest) {
        ...
        // 패스워드 일치 여부 확인
        if (!UserPasswordBuilder.comparePassword(password, userEntity.encryptedPassword)) {
